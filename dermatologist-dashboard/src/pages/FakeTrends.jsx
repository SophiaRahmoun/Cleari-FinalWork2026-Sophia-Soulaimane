          ...
          <div key={post.id} className="ft-card">
            <div className="ft-card-top">
              ...
              <div className="ft-card-body">
                ...
                {post.tiktokUrl && (
                  <TikTokPreview url={post.tiktokUrl} videoId={post.tiktokVideoId} />
                )}
              </div>
            </div>
...
function extractVideoId(url) {
  ...
  // https://vm.tiktok.com/XXXXX/ or https://vt.tiktok.com/XXXXX/
  const shortMatch = url.match(/tiktok\.com\/([A-Za-z0-9]+)\/?$/);
  return shortMatch ? shortMatch[1] : null;
}

function TikTokPreview({ url, videoId }) {
  const [showEmbed, setShowEmbed] = useState(false);
  const [embedFailed, setEmbedFailed] = useState(false);
  const iframeRef = useRef(null);

  const vid = videoId || extractVideoId(url);
  const embedSrc = vid ? `https://www.tiktok.com/embed/v2/${vid}` : null;

  if (!embedSrc || embedFailed) {
    return (
      <a href={url} target="_blank" rel="noopener noreferrer" className="ft-tiktok-card">
        <span className="ft-tiktok-icon">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
            <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.32 6.32 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.87a8.18 8.18 0 0 0 4.78 1.52V7a4.85 4.85 0 0 1-1.01-.31z"/>
          </svg>
        </span>
        View on TikTok
      </a>
  ...
  if (!showEmbed) {
    return (
      <button className="ft-tiktok-card ft-tiktok-card--btn" onClick={() => setShowEmbed(true)}>
        <span className="ft-tiktok-icon">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
            <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.32 6.32 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.87a8.18 8.18 0 0 0 4.78 1.52V7a4.85 4.85 0 0 1-1.01-.31z"/>
          </svg>
  ...
  return (
    <div className="ft-tiktok-embed-wrap">
      <iframe
        ref={iframeRef}
        src={embedSrc}
        className="ft-tiktok-iframe"
        allowFullScreen
        allow="autoplay"
        title="TikTok video"
        onError={() => setEmbedFailed(true)}
      />
      <div className="ft-tiktok-embed-footer">
        <a href={url} target="_blank" rel="noopener noreferrer" className="ft-tiktok-link">
          Open on TikTok
        </a>
        <button className="ft-tiktok-close" onClick={() => setShowEmbed(false)}>
          Close
        </button>
      </div>
    </div>
